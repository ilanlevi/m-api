...
export default class RedisConnector extends AbstractRedisConnection {
  ...
  public async queryForRedis(queryMapName: string, mapType: string, lastRequested?: number): Promise<any> {
    ...
    await this._redis.hgetall(fullQuery, (err, result) => {
      ...
      this._logger.info(`Query to ${fullQuery} ${lastRequested ? " (with lastRequested) " : ""} took: ${totalTime} ms.`);
      return result;
    })
  }
